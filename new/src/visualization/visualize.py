...
``analysis.analyze.run_full_analysis()``.

Individual plot functions each return a ``matplotlib.figure.Figure`` object.
The convenience function :func:`save_all_figures` runs all plots and writes
them to disk.
...
def _short_name(full_name: str, max_len: int = 20) -> str:
    """Shorten a model name for display. If it's already a display_name, pass through."""
...
def plot_confusion_matrix(
    ...
    ----------
    cm:
        Raw confusion matrix (int array, shape K×K).
    title:
        Figure title.
    normalise:
        If True, show row-normalised (recall) values with raw counts.
    """
    ...
    for i in range(len(labels)):
        for j in range(len(labels)):
            ...
            text = f"{data[i, j]:.1%}\n({cm[i, j]})" if normalise else str(cm[i, j])
...
# ---------------------------------------------------------------------------
# 2. Macro-F1 comparison bar chart
# ---------------------------------------------------------------------------
...
def plot_macro_f1_bar(
    ...
    Bootstrap 95% CI error bars shown when available.
    """
    ...
    if "model_name" not in metrics.columns or "prompt_id" not in metrics.columns:
        raise ValueError("plot_macro_f1_bar requires 'model_name' and 'prompt_id' columns.")
    ...
    if not ci_df.empty and ci_lower_col in ci_df.columns:
        ...
        metrics = metrics.merge(
            ci_df[merge_on + [ci_lower_col, ci_upper_col]], on=merge_on, how="left"
        )
    ...
    ax.set_title(
        f"{metric.replace('_', ' ').title()} by Model and Prompt Strategy", fontsize=12
    )
    ax.set_ylim(0, 1.05)
...
def plot_per_class_heatmap(
    ...
    """Heatmap of per-class metric — conditions on rows, classes on columns."""
    ...
    ax.set_title(f"Per-Class {metric.title()} by Condition{title_suffix}", fontsize=12)
...
def plot_bootstrap_ci(
    ...
    Conditions sorted by point estimate (descending).
    """
    ci_df = analysis.get("bootstrap_cis", pd.DataFrame())
    ...
    ax.set_title(f"{metric.replace('_', ' ').title()} with 95% Bootstrap CI", fontsize=12)
    ax.set_xlim(0, 1.0)
...
def plot_roc_curves(
    ...
    """Macro one-vs-rest ROC curves, one curve per model.
    ...
    if missing:
        warnings.warn(f"plot_roc_curves requires probability columns; missing: {missing}.")
        return None

    fig, ax = plt.subplots(figsize=figsize)
    palette = sns.color_palette("tab10")

    for g_idx, (group_key, group_df) in enumerate(results_df.groupby(group_col)):
        valid = group_df.dropna(subset=list(prob_cols.values()) + ["true_label"])
        if valid.empty:
            continue
...
def plot_reliability_diagram(
    ...
    """Reliability diagrams (one panel per condition). Returns None if no data."""
    ...
    for ax_idx, (key, cal) in enumerate(calibration.items()):
        ...
        ax.bar(bin_mids, frac_pos, width=0.08, alpha=0.5, color=color, edgecolor="white")
...
# ---------------------------------------------------------------------------
# 7. Holistic dashboard (multi-panel)
# ---------------------------------------------------------------------------
...
def plot_holistic_dashboard(
    ...
    """Comprehensive 2×2 multi-panel dashboard for at-a-glance model comparison.

    Layout::

        ┌─────────────────────┬─────────────────────────┐
        │  Macro-F1 bar chart │  Per-class F1 heatmap   │
        ├─────────────────────┼─────────────────────────┤
    ...
    if "model_name" in metrics.columns and "prompt_id" in metrics.columns:
        ...
        ax_bar.axhline(0.70, linestyle="--", color="grey", linewidth=0.8, alpha=0.7)
    ...
    ci_df = analysis.get("bootstrap_cis", pd.DataFrame())
    point_col, lo_col, hi_col = "macro_f1_point", "macro_f1_ci_lower", "macro_f1_ci_upper"
    ci_data = (
...
# ---------------------------------------------------------------------------
# NEW: API cost breakdown bar chart
# ---------------------------------------------------------------------------
...
def plot_cost_breakdown(
    ...
    """Bar chart of total API cost per model, grouped by prompt.

    Parameters
    ----------
    cost_df:
        Output of ``CostTracker.summary()`` or ``analyze.cost_summary()``.
        Required columns: ``model_name``, ``prompt_id``, ``total_cost_usd``.

    Returns
    -------
    matplotlib.figure.Figure
    """
    if cost_df.empty or "total_cost_usd" not in cost_df.columns:
        fig, ax = plt.subplots(figsize=figsize)
        ax.text(0.5, 0.5, "No API cost data available", ha="center", va="center",
                transform=ax.transAxes, fontsize=12)
        return fig

    models = cost_df["model_name"].unique()
    prompts = cost_df["prompt_id"].unique()
    n_models, n_prompts = len(models), len(prompts)
    x = np.arange(n_models)
    bar_width = 0.8 / n_prompts
    ...
    fig, ax = plt.subplots(figsize=figsize)
    for p_idx, prompt in enumerate(prompts):
        sub = cost_df[cost_df["prompt_id"] == prompt].set_index("model_name")
        values = [float(sub.loc[m, "total_cost_usd"]) if m in sub.index else 0.0
                  for m in models]
        offsets = x - 0.4 + bar_width * (p_idx + 0.5)
        color = PROMPT_COLORS.get(str(prompt), None)
        ax.bar(offsets, values, width=bar_width, label=str(prompt),
               color=color, edgecolor="white", linewidth=0.5)

    ax.set_xticks(x)
    ax.set_xticklabels([_short_name(m) for m in models], rotation=20, ha="right")
    ax.set_ylabel("Total Cost (USD)", fontsize=11)
    ax.set_title("API Inference Cost by Model and Prompt Strategy", fontsize=12)
    ax.legend(title="Prompt", bbox_to_anchor=(1.01, 1), loc="upper left", fontsize=9)
    fig.tight_layout()
    return fig
...
def save_all_figures(
    ...
    ----------
    analysis:
        Output of :func:`src.analysis.analyze.run_full_analysis`.
    output_dir:
        Directory where figures are saved (created if absent).
    results_df:
        Raw results DataFrame (enables ROC curves if probability columns exist).
    cost_df:
        Cost summary DataFrame from CostTracker (enables cost breakdown figure).
    fmt:
        File format: ``"pdf"`` (vector, best for publication), ``"png"``, ``"svg"``.
    """
    out = Path(output_dir)
    ...
    if "prompt_id" in analysis["per_class_metrics"].columns:
        for prompt_id in analysis["per_class_metrics"]["prompt_id"].unique():
            try:
                _save(
                    plot_per_class_heatmap(analysis, metric="f1", prompt_filter=prompt_id),
                    f"per_class_f1_{prompt_id}",
                )
    ...
    if results_df is not None:
        _save(plot_roc_curves(results_df), "roc_curves")

    _save(plot_reliability_diagram(analysis), "reliability_diagram")

    if cost_df is not None and not cost_df.empty:
        _save(plot_cost_breakdown(cost_df), "api_cost_breakdown")
